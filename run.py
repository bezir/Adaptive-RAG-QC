# Adaptive-RAG HP manager and runner
#
# What this script does
# - Instantiates base Jsonnet configs by expanding local variables and hyperparameters (see `instantiation_schemes`).
# - Writes, diffs, and verifies instantiated configs; manages prediction/evaluation runs end-to-end.
# - Tracks progress, summarizes metrics, and backs up/restores prediction directories.
#
# Key concepts
# - Base config: a .jsonnet experiment file passed via `experiment_name_or_path`.
# - Instantiation scheme: chooses values for variables like `bm25_retrieval_count`, `distractor_count` to create variants.
# - Prompt sets: {1, 2, 3, aggregate}; mapped to `valid_qids` by dataset inferred from the config (`infer_dataset`).
#
# Inputs / Outputs
# - Reads: base .jsonnet config, evaluation file, and environment for retriever/LLM addresses.
# - Writes:
#   - Instantiated configs under `instantiated_configs/`
#   - Predictions under `predictions/<experiment_name>/prediction__*.json`
#   - Metrics under `predictions/<experiment_name>/evaluation_metrics__*.json`
#   - Ground-truth copies under `predictions/<experiment_name>/ground_truth__*.json`
#
# Common commands (examples)
# - Write all variants:   python run.py <config.jsonnet> --instantiation_scheme ircot --prompt_set 1 --set_name <tag> --llm_port_num <port> write
# - Predict:              python run.py <config.jsonnet> --instantiation_scheme ircot --prompt_set 1 --set_name <tag> --llm_port_num <port> predict --evaluation_path <path>
# - Evaluate:             python run.py <config.jsonnet> --instantiation_scheme ircot --prompt_set 1 --set_name <tag> --llm_port_num <port> evaluate --evaluation_path <path>
# - Summarize best:       python run.py <config.jsonnet> --instantiation_scheme ircot --prompt_set 1 --set_name <tag> --llm_port_num <port> summarize --evaluation_path <path> --best
# - Track completion:     python run.py <config.jsonnet> --instantiation_scheme ircot --prompt_set 1 --set_name <tag> --llm_port_num <port> track --evaluation_path <path>
# - Backup/Recover preds: python run.py <config.jsonnet> --instantiation_scheme ircot --prompt_set 1 --set_name <tag> --llm_port_num <port> backup|recover_backup
#
# Environment & caching
# - Uses HuggingFace cache at `{PROJECT_ROOT}/Adaptive-RAG/.cache/huggingface` (set via `HF_CACHE`).
# - Designed to run inside the `adaptiverag` conda environment.
#
# Notes
# - Best HP is selected based on prompt_set=1 and reused for sets 2/3 or aggregate summarize.
# - For `aggregate`, only summarize with `--best` is supported.
...
#os.environ['HF_CACHE'] = f"set_your_cache_path_here"
