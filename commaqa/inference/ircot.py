...
def safe_post_request(url, params):
    for attempt in range(3):  # Reduced from 10 to 3 attempts
        try:
            response = requests.post(url, json=params, timeout=10)  # Added timeout
            if response.status_code == 200:
                return response
            else:
                print(f"Post request failed with status {response.status_code}. Attempt {attempt + 1}/3")
        except requests.exceptions.RequestException as e:
            print(f"Post request didn't succeed. Attempt {attempt + 1}/3. Error: {str(e)}")
        except Exception as e:
            print(f"Unexpected error in post request. Attempt {attempt + 1}/3. Error: {str(e)}")
        
        if attempt < 2:  # Don't sleep on last attempt
            time.sleep(2)  # Reduced from 20s to 2s
    
    raise Exception("Post request couldn't succeed after 3 attempts.")
...
class RetrieveAndResetParagraphsParticipant(ParticipantModel):
    ...
    def reset_state(self):
        """Reset participant state between different questions"""
        self.num_calls = 0
        self.retrieval_failures_so_far = 0  # Reset retrieval failures as well
...
class CopyQuestionParticipant(ParticipantModel):
    ...
    def query(self, state, debug=False):
        # Add comprehensive debug prints to trace termination behavior
        if debug:
            print(f"DEBUG: CopyQuestionParticipant.query called")
            print(f"DEBUG: num_calls: {self.num_calls}")
            print(f"DEBUG: eoq_after_n_calls: {self.eoq_after_n_calls}")
            print(f"DEBUG: next_model: {self.next_model}")
            print(f"DEBUG: end_state: {self.end_state}")
            
        termination_check = (self.num_calls + 1) % (self.eoq_after_n_calls + 1) == 0
        
        if debug:
            print(f"DEBUG: Termination check: ({self.num_calls} + 1) % ({self.eoq_after_n_calls} + 1) = {(self.num_calls + 1)} % {(self.eoq_after_n_calls + 1)} = {(self.num_calls + 1) % (self.eoq_after_n_calls + 1)}")
            print(f"DEBUG: Should terminate: {termination_check}")

        if termination_check:
            output = self.end_state
            if debug:
                print(f"DEBUG: TERMINATING - output set to end_state: '{output}'")
        else:
            output = state.data["question"].strip()
            if debug:
                print(f"DEBUG: NOT TERMINATING - output set to question: '{output[:100]}...'")

        self.num_calls += 1
        if debug:
            print(f"DEBUG: Incremented num_calls to: {self.num_calls}")
...
class StepByStepLLMTitleGenParticipant(ParticipantModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if gen_model == "llm_api":
            self.generator = LLMClientGenerator(**kwargs)
        elif gen_model == "gemini":
            self.generator = GeminiGenerator(**kwargs)
        elif gen_model == "qwen":
            # Map engine parameter to model_name for Qwen models
            if "engine" in kwargs:
                engine_name = kwargs.pop("engine")
                # Map short engine names to full model names expected by LLM server
                if engine_name == "qwen2.5-7b-instruct":
                    kwargs["model_name"] = "Qwen/Qwen2.5-7B-Instruct"
                else:
                    kwargs["model_name"] = engine_name
            self.generator = LLMClientGenerator(**kwargs)
        else:
...
class StepByStepCOTGenParticipant(ParticipantModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if gen_model == "llm_api":
            ...
            self.generator = GeminiGenerator(**kwargs)
            # CRITICAL FIX: Override default stop tokens to enable reasoning
            if hasattr(self, 'generator') and hasattr(self.generator, 'stop'):
                # Force override to remove newline stop tokens
                if 'stop' in kwargs and kwargs['stop'] is None:
                    self.generator.stop = None
                    print(f"DEBUG: Overrode GeminiGenerator stop tokens to None")
                elif 'stop' in kwargs:
                    self.generator.stop = kwargs['stop']
                    print(f"DEBUG: Set GeminiGenerator stop tokens to: {kwargs['stop']}")
                print(f"DEBUG: Final GeminiGenerator stop tokens: {self.generator.stop}")
        elif gen_model == "qwen":
            # Map engine parameter to model_name for Qwen models
            if "engine" in kwargs:
                engine_name = kwargs.pop("engine")
            ...
            # Filter kwargs to only include parameters that LLMClientGenerator accepts
            allowed_params = {
                'model_name', 'max_input', 'max_length', 'min_length', 'do_sample',
                'eos_text', 'temperature', 'top_k', 'top_p', 'num_return_sequences',
                'repetition_penalty', 'length_penalty', 'model_tokens_limit', 'remove_method'
            }
            
            # Convert common parameter names
            filtered_kwargs = {}
            for key, value in kwargs.items():
                if key == 'max_tokens':
                    filtered_kwargs['max_length'] = value
                elif key == 'stop':
                    if isinstance(value, list) and value:
                        filtered_kwargs['eos_text'] = value[0]
                    elif isinstance(value, str):
                        filtered_kwargs['eos_text'] = value
                elif key in allowed_params:
                    filtered_kwargs[key] = value
            
            self.generator = LLMClientGenerator(**filtered_kwargs)
        else:
            raise ValueError("Unknown gen_model: " + gen_model)
        ...
        self.answer_extractor_regex = re.compile(answer_extractor_regex, re.IGNORECASE)  # FIXED: Case-insensitive
    ...
    def reset_state(self):
        """Reset participant state between different questions"""
        self.num_calls = 0
        # Also reset the spaCy nlp cache to prevent potential cross-contamination
        if hasattr(self, 'nlp'):
            self.nlp.max_length = 10000000  # Reset any potential state

    def return_model_calls(self):
        return {"step_by_step_cot_gen": self.num_calls}
    ...
    def query(self, state, debug=False):
        ...
        if not exit_generation:
            ...
            # CRITICAL FIX: Add unique identifier to break cache contamination
            import time
            import random
            cache_breaker = f"<!-- ID: {int(time.time() * 1000000) % 1000000}{random.randint(100, 999)} -->"
            ...
            # ENHANCED: Check if we should prompt for final answer when sufficient information is gathered
            final_answer_prompt_added = False
            num_sentences = len(state.data[f'generated_{self.generation_type}'])
            if num_sentences >= 3:  # After a few reasoning steps
                try:
                    ...
                    # Simple pattern matching for sufficient information detection
                    generation_lower = generation_so_far.lower()
                    question_lower = question.lower()
                    
                    # Check for sufficient information patterns
                    has_sufficient_info = False
                    if 'spouse' in question_lower and any(term in generation_lower for term in ['married', 'wife', 'husband']):
                        has_sufficient_info = True
                    elif 'war' in question_lower and any(term in generation_lower for term in ['war', 'conflict', 'served']):
                        has_sufficient_info = True
                    elif 'actor' in question_lower and 'plays' in generation_lower:
                        has_sufficient_info = True
                    
                    if has_sufficient_info:
                        # Add explicit final answer prompt with format instruction to guide model
                        test_example_str += "\n\n# METADATA: {\"qid\": \"final_step\"}\nQ: Provide the final answer.\nA: Based on my reasoning above,"
                        final_answer_prompt_added = True
                        if debug:
                            print(f"DEBUG: SUFFICIENT INFO DETECTED - Added explicit final answer prompt")
                except Exception as e:
                    if debug:
            ...
            if debug:
                print(f"DEBUG: CoT Generation - sentences so far: {len(state.data[f'generated_{self.generation_type}'])}")
                print(f"DEBUG: Generation so far preview: '{generation_so_far[-200:]}...'")
                print(f"DEBUG: Prompt length: {len(prompt)} chars")
                if final_answer_prompt_added:
                    print(f"DEBUG: FINAL ANSWER PROMPT INJECTED")

            output_text_scores = self.generator.generate_text_sequence(prompt)
            if len(output_text_scores) > 1:
                print("Can not handle more than one answer for this model yet" + "\n" + str(output_text_scores))

            # CRITICAL FIX: Handle None responses from Gemini API
            raw_text = output_text_scores[0][0]
            if raw_text is None:
                print(f"IRCOT DEBUG: WARNING: Generator returned None - treating as empty generation")
                new_generation = ""
                new_sents = []
            else:
                new_generation = raw_text.strip()
                print(f"IRCOT DEBUG: Raw generation: '{new_generation}'")
                new_sents = list(self.spacy_object(new_generation).sents)
            print(f"IRCOT DEBUG: spaCy detected {len(new_sents)} sentences")
            if new_sents:
                # CRITICAL FIX: Check ALL sentences for answer pattern, not just first one
                # Use STRICT local regex to match exit controller pattern EXACTLY
                answer_patterns = [
                    re.compile(r"^\s*(?:So\s+)?the\s+answer\s+is:?\s*(.*)", re.IGNORECASE),  # STRICT: Force start of sentence
                    re.compile(r"^\s*answer\s+is:?\s*(.*)", re.IGNORECASE)  # STRICT: Force start of sentence
                ]
                
                # Check for answer patterns in generated sentences
                answer_found = False
                ...
                for i, sent in enumerate(new_sents):
                    ...
                    answer_match = self.answer_extractor_regex.match(sentence_text)
                    if answer_match:
                        print(f"IRCOT DEBUG: ANSWER FOUND in sentence {i+1}: '{sentence_text}'")
                        
                        # IRCOT should be able to answer simple questions in 1 step like ONER
                        # No forced minimum - let the LLM decide when it has enough information
                        answer_found = True
                        answer_sentence = sentence_text
                        break
                
                if answer_found:
                    # Use the answer sentence - the exit controller will handle termination
                    new_generation = answer_sentence
                    print(f"IRCOT DEBUG: Using ANSWER sentence: '{new_generation}'")
                    print(f"IRCOT DEBUG: ANSWER DETECTED - Will let exit controller handle termination!")
                    print(f"IRCOT DEBUG: Exit controller will extract: '{answer_match.group(1)}'")
                    # DO NOT set exit_generation = True here - let exit controller decide
                else:
                    # CRITICAL FIX: Smart sentence selection instead of always using first sentence
                    # PRIORITY 1: Answer sentences (for termination)
                    # PRIORITY 2: Informative sentences (for reasoning)
                    selected_sentence = None
                    
                    # HIGHEST PRIORITY: Look for answer sentences first (but respect premature rejection)
                    # CRITICAL FIX: Use the same regex pattern as exit controller
                    answer_pattern = r"^\s*(?:So\s+)?the\s+answer\s+is:?\s*(.*)"  # STRONGER: Force start of sentence
                    answer_regex = re.compile(answer_pattern, re.IGNORECASE)
                    print(f"IRCOT DEBUG: CoT generator using FIXED answer regex pattern: '{answer_pattern}'")
                    
                    # IRCOT should be able to answer simple questions in 1 step like ONER
                    for sent in new_sents:
                        sentence_text = sent.text.strip()
                        print(f"IRCOT DEBUG: Checking sentence for answer pattern: '{sentence_text}'")
                        answer_match = answer_regex.match(sentence_text)
                        print(f"IRCOT DEBUG: Answer regex match result: {answer_match}")
                        if answer_match:
                            selected_sentence = sentence_text
                            print(f"IRCOT DEBUG: Selected ANSWER sentence (highest priority): '{selected_sentence}'")
                            break
                    
                    # SECOND PRIORITY: Look for informative sentences if no answer found
                    if not selected_sentence:
                        for sent in new_sents:
                            sentence_text = sent.text.strip()
                            
                            # Skip multiple types of non-informative sentences
                            skip_patterns = [
                                r"^I need to find",
                                r"^I need to .*",
                                r"^Let me find",
                                ...
                                r"^I will find",
                                r"^I will .*",
                                r"^Q\s*:",  # Question restatements
                                r"^A\s*:\s*I need",  # Answer prefixes with search intent
                                r".*\?$",  # Any sentence ending with question mark
                                r"^Now I need to",  # Transitional planning sentences
                                r"^I need to find out",  # More search intent patterns
                            ...
                            is_non_informative = any(re.match(pattern, sentence_text, re.IGNORECASE) for pattern in skip_patterns)
                            
                            # Additional check: avoid sentences that are just question restatements
                            words = sentence_text.split()
                            is_too_short = len(words) < 4
                            
                            if not is_non_informative and not is_too_short:  # Must be substantial and informative
                                selected_sentence = sentence_text
                                print(f"IRCOT DEBUG: Selected INFORMATIVE sentence: '{selected_sentence}'")
                                break
                    
                    # ENHANCED FALLBACK: if no good sentence found, force answer synthesis
                    if not selected_sentence:
                        existing_sentences = new_state.data.get(f"generated_{self.generation_type}", [])
                        if len(existing_sentences) >= 1:
                            # We have some reasoning - try to synthesize an answer
                            selected_sentence = "Based on the available information, I cannot determine a definitive answer from the provided context."
                            print(f"IRCOT DEBUG: No good sentences found - forcing answer synthesis: '{selected_sentence}'")
                        else:
                            # No reasoning yet - use first sentence but log the issue
                            selected_sentence = new_sents[0].text
                            print(f"IRCOT DEBUG: WARNING: Using first sentence (no alternatives): '{selected_sentence}'")
                            # Check if this is a question - if so, try to force early termination in the next step
                            if any(re.match(pattern, selected_sentence, re.IGNORECASE) for pattern in [r".*\?$", r"^Q\s*:", r"^What\s+", r"^Which\s+", r"^How\s+", r"^When\s+", r"^Where\s+", r"^Who\s+"]):
                                print(f"IRCOT DEBUG: WARNING: Selected sentence is a question - may force termination soon")
                    
                    new_generation = selected_sentence
                
                # CRITICAL FIX: Check for repetitive generation to prevent infinite loops
                existing_sentences = new_state.data.get(f"generated_{self.generation_type}", [])
                new_generation_stripped = new_generation.strip()
                
                # ENHANCED REPETITION DETECTION - Check both exact repetition and planning loops
                is_repetitive = False
                
                # Check for exact repetition
                if len(existing_sentences) >= 3:  
                    last_sentence = existing_sentences[-1].strip()
                    if last_sentence == new_generation_stripped and len(new_generation_stripped) > 10:
                        print(f"IRCOT DEBUG: WARNING: EXACT REPETITIVE SENTENCE DETECTED: '{new_generation_stripped[:100]}...'")
                        is_repetitive = True
                
                # CRITICAL FIX: Check for repetitive planning patterns that cause loops
                if len(existing_sentences) >= 2:
                    planning_patterns = [
                        "I need to find", "I will compare", "Then I will", "Now I need to",
                        "I should find", "Let me find", "I must find", "I have to find",
                        "I need to identify", "I need to determine"
                    ]
                    
                    # Count planning sentences in recent history
                    recent_sentences = existing_sentences[-3:] + [new_generation_stripped]
                    planning_count = sum(1 for sentence in recent_sentences 
                                       if any(sentence.startswith(pattern) for pattern in planning_patterns))
                    
                    # If 3+ planning sentences in last 4 attempts, it's a planning loop
                    if planning_count >= 3:
                        print(f"IRCOT DEBUG: WARNING: REPETITIVE PLANNING LOOP DETECTED")
                        print(f"IRCOT DEBUG: Planning sentences: {planning_count}/4 recent sentences")
                        print(f"IRCOT DEBUG: Last sentence: '{new_generation_stripped[:100]}...'")
                        is_repetitive = True
                
                if is_repetitive:
                    # Instead of giving up, try to force an answer based on what we know
                    print(f"IRCOT DEBUG: REPETITION DETECTED - ATTEMPTING TO SYNTHESIZE ANSWER")
                    # Try to create an answer from the information we've gathered so far
                    reasoning_so_far = " ".join(existing_sentences)
                    print(f"IRCOT DEBUG: Reasoning so far: '{reasoning_so_far[:200]}...'")
                    
                    # CRITICAL FIX: Force answer generation when we have sufficient info
                    # Check if we have informative content that could lead to an answer
                    if len(existing_sentences) >= 2:
                        # Try to prompt for a direct answer synthesis
                        new_generation = f"Based on the information gathered, the answer is:"
                        print(f"IRCOT DEBUG: FORCING ANSWER SYNTHESIS: '{new_generation}'")
                    else:
                        new_generation = "Unable to find sufficient information to answer the question."
                        print(f"IRCOT DEBUG: INSUFFICIENT INFO - Using fallback termination")
                else:
                    # Use the selected informative sentence
                    new_generation = selected_sentence
                    print(f"IRCOT DEBUG: Using selected informative sentence: '{new_generation[:100]}...'")
                
                print(f"IRCOT DEBUG: Final sentence selection complete")
                
                # CRITICAL FIX: Add the selected sentence to the reasoning chain
                new_state.data[f"generated_{self.generation_type}"].append(new_generation)
                print(f"IRCOT DEBUG: Added sentence to list. Total sentences now: {len(new_state.data[f'generated_{self.generation_type}'])}")

        else:
            print(f"IRCOT DEBUG: No sentences detected by spaCy!")
            # Add empty sentence to signal termination to exit controller
            new_state.data[f"generated_{self.generation_type}"].append("")
            print(f"IRCOT DEBUG: Added empty sentence - exit controller will handle termination")

        # CRITICAL FIX: CoT generator should ALWAYS go to next model (exit controller)
        # The exit controller will decide whether to terminate or continue
        # Remove the termination logic from CoT generator - that's the exit controller's job
        
        # Always output the full reasoning chain so far
        output = " ".join(new_state.data[f"generated_{self.generation_type}"])
        new_state.next = self.next_model
        
        print(f"IRCOT DEBUG: CoT generator always going to next model: {self.next_model}")
        print(f"IRCOT DEBUG: Let exit controller decide termination based on: '{output[-100:]}...'")

        # Note: Removed all exit_generation termination logic - exit controller handles this
...
class StepByStepExitControllerParticipant(ParticipantModel):
    ...
    def __init__(
        self,
        answer_extractor_regex=r"^\s*(?:So\s+)?the\s+answer\s+is:?\s*(.*)",  # STRONGER: Force start of sentence
        answer_extractor_remove_last_fullstop=True,
        terminal_return_type="titles",
        max_num_sentences=10,
        terminal_state_next_model=None,
        ...
        self.num_calls = 0
        self.answer_extractor_regex = re.compile(answer_extractor_regex, re.IGNORECASE)  # FIXED: Case-insensitive
        # CRITICAL DEBUG: Show exactly what regex pattern was compiled
        print(f"EXIT CONTROLLER DEBUG: Compiled answer regex: '{answer_extractor_regex}'")
        print(f"EXIT CONTROLLER DEBUG: Pattern object: {self.answer_extractor_regex}")
        # Test the regex with our expected pattern
        test_text = "So the answer is: 1862"
        test_match = self.answer_extractor_regex.match(test_text)
        if test_match:
            print(f"EXIT CONTROLLER DEBUG: Test match successful: '{test_match.group(1)}'")
        else:
            print(f"EXIT CONTROLLER DEBUG: Test match FAILED for: '{test_text}'")
        self.answer_extractor_remove_last_fullstop = answer_extractor_remove_last_fullstop
        self.terminal_return_type = terminal_return_type
        self.max_num_sentences = max_num_sentences
        self.terminal_state_next_model = terminal_state_next_model
    ...
    def query(self, state, debug=False):
        ...
        # ENHANCED: Check for answer patterns with detailed debugging
        if generated_sentences:
            last_sentence = generated_sentences[-1]
            print(f"EXIT CONTROLLER DEBUG: Checking last sentence for answer pattern: '{last_sentence[:100]}...'")
            
            answer_match = self.answer_extractor_regex.match(last_sentence)
            if answer_match:
                return_answer = answer_match.group(1)
                if self.answer_extractor_remove_last_fullstop and return_answer.endswith("."):
                    return_answer = return_answer[:-1]
                exit_generation = True  # CRITICAL FIX: Move inside the if block!
                print(f"EXIT CONTROLLER DEBUG: ANSWER PATTERN MATCHED! Extracted: '{return_answer}'")
                print(f"EXIT CONTROLLER DEBUG: Setting exit_generation = True")
            else:
                print(f"EXIT CONTROLLER DEBUG: No answer pattern matched in last sentence")
                
                # ADDITIONAL: Check if any sentence contains forced termination indicators
            if "Unable to find sufficient information" in last_sentence:
                print(f"EXIT CONTROLLER DEBUG: WARNING: Forced termination detected - no sufficient information")
                exit_generation = True
                return_answer = "Unable to determine answer from available information"

        if exit_generation:
            print(f"EXIT CONTROLLER DEBUG: TERMINATING IRCOT - exit_generation = True")
            print(f"EXIT CONTROLLER DEBUG: Total sentences generated: {len(generated_sentences)}")
            print(f"EXIT CONTROLLER DEBUG: Termination reasons checked:")
            print(f"EXIT CONTROLLER DEBUG:   - Empty last sentence: {generated_sentences and not generated_sentences[-1]}")
            print(f"EXIT CONTROLLER DEBUG:   - Max sentences reached: {len(generated_sentences) >= self.max_num_sentences} ({len(generated_sentences)}/{self.max_num_sentences})")
            print(f"EXIT CONTROLLER DEBUG:   - Answer pattern matched: {generated_sentences and self.answer_extractor_regex.match(generated_sentences[-1]) is not None}")
            ...
            print(f"EXIT CONTROLLER DEBUG: Final output ({self.terminal_return_type}): '{output[:100]}...'")

            if self.terminal_state_next_model is not None:
                new_state.next = self.terminal_state_next_model
                print(f"EXIT CONTROLLER DEBUG: Next model: {self.terminal_state_next_model}")
            else:
                new_state.next = self.end_state
                print(f"EXIT CONTROLLER DEBUG: Next state: {self.end_state}")

        else:
            print(f"EXIT CONTROLLER DEBUG: CONTINUING IRCOT - exit_generation = False")
            print(f"EXIT CONTROLLER DEBUG: Current sentences: {len(generated_sentences)}, max allowed: {self.max_num_sentences}")
            output = "Exit? No."
            new_state.next = self.next_model
            print(f"EXIT CONTROLLER DEBUG: Next model: {self.next_model}")
            # Don't add control messages to the reasoning chain
            return [new_state]
