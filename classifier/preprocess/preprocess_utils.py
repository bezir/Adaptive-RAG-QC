...
def label_complexity(orig_file_path, zero_file_path, one_file_path, multi_file_path, dataset_name):
    ...
    # Load prediction files once, outside the loop, for efficiency.
    dict_zero = load_json(zero_file_path)
    dict_one = load_json(one_file_path)
    dict_multi = load_json(multi_file_path)

    # Get lists of successfully answered question IDs.
    lst_multi_qid = list(dict_multi.keys())
    lst_one_qid = list(dict_one.keys())
    lst_zero_qid = list(dict_zero.keys())
