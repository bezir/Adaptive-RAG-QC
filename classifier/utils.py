...
def load_model(args):
    ...
    # Detect model type based on model name or config
    model_name = args.model_name_or_path.lower()
    is_bert_model = (
        'bert' in model_name or 
        'roberta' in model_name or 
        'distilbert' in model_name or
        (hasattr(config, 'model_type') and config.model_type in ['bert', 'roberta', 'distilbert'])
    )
    
    if args.model_name_or_path:
        if is_bert_model:
            # Set num_labels in config before loading the model
            config.num_labels = 3  # A, B, C classes
            # Use sequence classification for BERT-style models
            ...
            # Use seq2seq for T5-style models
        ...
        if is_bert_model:
            model = AutoModelForSequenceClassification.from_config(config, num_labels=3)
        else:
            model = AutoModelForSeq2SeqLM.from_config(config)
        
    return model, tokenizer, is_bert_model
...
def preprocess_features_function_bert(examples, args, raw_datasets, tokenizer):
    """Preprocess features for BERT-style classification models"""
    question_column, answer_column = preprocess_dataset(args, raw_datasets)

    padding = "max_length" if args.pad_to_max_length else False
    max_seq_length = min(args.max_seq_length, tokenizer.model_max_length)

    # Remove whitespace from questions
    examples[question_column] = ['{}'.format(q.strip()) for q in examples[question_column]]

    # Tokenize questions for classification
    ...
    # Convert answer labels to integers (handle empty answers for test data)
    option_to_label = {'A': 0, 'B': 1, 'C': 2}
    labels = []
    for answer in examples[answer_column]:
        if answer in option_to_label:
            labels.append(option_to_label[answer])
        else:
            # For test data or missing answers, use -1 as placeholder
            labels.append(-100)

    # Handle overflow tokens
    sample_mapping = model_inputs.pop("overflow_to_sample_mapping")
    
    # Add example IDs and labels
    ...
    for i in range(len(model_inputs["input_ids"])):
        ...
        labels_out.append(labels[sample_index])
...
def preprocess_features_function(examples, args, raw_datasets, tokenizer):
    """Original preprocessing function for T5-style models"""
    question_column, answer_column = preprocess_dataset(args, raw_datasets)
