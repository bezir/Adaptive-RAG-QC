...
class ElasticsearchRetriever:
    ...
    def __init__(self, host: str = "localhost", port: int = 9200):
        ...
        # Modern Elasticsearch client expects host in URL format
        ...
        if host.startswith(('http://', 'https://')):
            ...
            # No protocol specified, add it
            full_host = f"http://{host}:{port}"
        
        # CRITICAL FIX: Create isolated ES client for each retriever instance
        self._es = Elasticsearch([full_host], timeout=30, 
                                # Force new connection pool for isolation
        ...
        self._instance_id = f"es_client_{id(self)}_thread_{thread_id}"
