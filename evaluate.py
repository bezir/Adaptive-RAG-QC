...
This script provides comprehensive evaluation capabilities for the Adaptive RAG system,
supporting multiple evaluation metrics, official evaluation protocols, and detailed
analysis of model predictions across different tasks and datasets.

Key Features:
1. Multi-Metric Evaluation
   - Exact Match (EM): Binary accuracy for exact answer matches
   - F1 Score: Token-level overlap between prediction and gold answers
   - Precision/Recall: Fine-grained answer quality assessment
   - Support metrics: Evaluation of supporting evidence quality

2. Dataset-Specific Evaluation
   - HotpotQA: Multi-hop reasoning with supporting facts evaluation
   - 2WikiMultiHopQA: Wikipedia-based multi-hop reasoning
   - MuSiQue: Multi-hop with unanswerable question handling
   - Natural Questions: Real user questions with Wikipedia context
   - TriviaQA: Trivia questions with multiple evidence sources
   - SQuAD: Reading comprehension with paragraph context

3. Official Evaluation Integration
   - Native support for official evaluation scripts
   - Automated temporary file management
   - Result format conversion and standardization
   - Cross-validation with official metrics

4. Answer Processing Pipeline
   - Chain-of-thought answer extraction
   - Text normalization and cleaning
   - Multi-answer prediction handling
   - Answer format standardization

5. Prediction Type Support
   - Answer predictions: Final answer text evaluation
   - Title predictions: Supporting document title evaluation
   - Paragraph predictions: Retrieved passage evaluation
   - Step-by-step reasoning evaluation

Evaluation Metrics:
- Answer EM/F1: Primary accuracy metrics for answer quality
- Support EM/F1: Metrics for supporting evidence accuracy
- Answer-Support Recall: Coverage of relevant supporting evidence
- Official metrics: Dataset-specific evaluation protocols

Supported Answer Formats:
- Single answers: "Paris"
- List answers: ["Paris", "France"] 
- Chain-of-thought: "The capital of France is Paris."
- Multi-step reasoning: Step-by-step derivations
...
Official Evaluation Support:
- HotpotQA: hotpot_evaluate_v1.py integration
- 2WikiMultiHopQA: 2wikimultihop_evaluate_v1.1.py integration  
- MuSiQue: evaluate_v1.0.py integration
- Automatic format conversion and result parsing

Multi-Step Retrieval Analysis:
- Generated title tracking across retrieval steps
- Projected title analysis for retrieval effectiveness
- Final title selection evaluation
- Chain-of-thought reasoning step analysis

The script enables comprehensive assessment of RAG system performance across:
1. Answer accuracy and quality
2. Supporting evidence retrieval effectiveness  
3. Multi-hop reasoning capability
4. Cross-dataset generalization
5. Official benchmark compliance

Usage Examples:
```bash
# Evaluate predictions with internal metrics
python evaluate.py config.jsonnet evaluation_data.jsonl

# Run official evaluation
python evaluate.py config.jsonnet evaluation_data.jsonl --official

# Evaluate specific prediction types
python evaluate.py config.jsonnet evaluation_data.jsonl --prediction_type titles
```

The evaluation script provides the foundation for rigorous assessment of Adaptive RAG
system performance, enabling detailed analysis and comparison across different
retrieval strategies and model configurations.
"""
